...
def db_auth_and_save_dialog(engine, current_table, pk_cols, edited_rows, added_rows, deleted_rows):
    ...
    if st.button("인증 및 저장 실행"):
        ...
        try:
            # Create a temporary engine with the provided credentials
...
def render_db_editor(engine):
    st.header("DB 테이블 관리 및 편집")
    st.markdown("아래 **데이터베이스 관계도(ERD)**에서 테이블을 선택하고, 데이터를 직접 편집하세요. 저장 시 Root 권한이 필요합니다.")
    ...
    if query_params_table and query_params_table in all_db_tables:
        st.session_state['selected_erd_table_admin'] = query_params_table
    elif 'selected_erd_table_admin' not in st.session_state or st.session_state['selected_erd_table_admin'] not in all_db_tables:
        st.session_state['selected_erd_table_admin'] = "part"
        
    current_table = st.session_state['selected_erd_table_admin']
    ...
    tab_data, tab_schema = st.tabs(["📑 테이블 데이터 편집", "📐 테이블 스키마 및 관계 정의"])
    ...
    with tab_data:
        try:
            with engine.connect() as conn:
                ...
                with f1:
                    search_kw = st.text_input("🔍 데이터 검색 (키워드)", placeholder="검색할 값을 입력하세요...", key=f"search_admin_{current_table}")
                with f2:
                    row_limit = st.selectbox("조회 행 수 제한", [100, 300, 500, 1000, "전체"], index=0, key=f"limit_admin_{current_table}")
                with f3:
                    st.write("")
                    st.write("")
                ...
                # We save original_df to session state so we can map row indices back to PKs on save
                st.session_state[f"original_df_{current_table}"] = df.copy()
                
                if df.empty:
                    st.warning("선택한 테이블에 데이터가 없거나 검색 조건에 일치하는 결과가 없습니다.")
                    # Allow inserting into empty table
            ...
            st.error(f"데이터 편집 중 오류가 발생했습니다: {e}")

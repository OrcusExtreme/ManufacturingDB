    ...
    for line in f:
        ...
        if data.get('type') == 'TOOL_RESPONSE' and 'xml_parser.py' in data.get('content', ''):
            if 'Total Bytes' in data.get('content'):
                print("Found match in step", data.get('step_index'))
                content = data['content']
                with open(rf"d:\Project_Lab\Lab Database\Project\xml_parser_step_{data.get('step_index')}.txt", "w", encoding='utf-8') as out:
                    out.write(content)
